...
interface QianwenConversationRequest {
...
interface QianwenHistoryMessageInput {
  role?: unknown;
  content?: unknown;
  created_at?: unknown;
  createdAt?: unknown;
...
export function parseQianwenRequestBody(body: string): NormalizedMessage[] {
  const parsed = JSON.parse(body) as QianwenConversationRequest;
...
export function parseQianwenSseResponse(body: string): NormalizedMessage[] {
  ...
  for (const line of body.split('\n')) {
    ...
    const parsed = safeParseJson<QianwenSseEvent>(raw);
    if (!parsed) {
      continue;
    }
...
export function parseQianwenHistoryResponse(body: string): NormalizedMessage[] {
...
export function summarizeQianwenResponseBody(body: string, maxLength = 400): string {
...
function extractAssistantContent(input: unknown): {
  ...
  const content =
    ...
    extractQianwenResponseTextFromResponseNode(input) ??
    '';
...
function extractConversationIdFromRequest(input: QianwenConversationRequest): string | undefined {
  return (
...
function extractQianwenResponseTextFromResponseNode(input: unknown): string | null {
